                ...
                return (
                    ...
                    >
                        <div className={"flex items-center gap-2 w-full"}>
                            <div className={"relative"}>
                                {user.image ? (
                                    ...
                                    <div className={"size-4 rounded-full bg-gray-400 flex items-center justify-center"}>
                                        <span
                                            className={"text-xs text-white"}>{(user.name || user.id).charAt(0).toUpperCase()}</span>
                                    </div>
                                )}

                                <CircleIcon className={`size-2 absolute -bottom-0.5 -right-0.5 ${
                            ...
                            <span
                                className={"str-chat__channel-preview-messenger--name truncate"}>{user.name || user.id}</span>

                            {unreadCount > 0 && (
                                <span
                                    className={"flex items-center justify-center ml-2 size-4 text-xs rounded-full bg-red-500"}>{unreadCount}</span>
                            )}
                        </div>
                    </button>
                )
