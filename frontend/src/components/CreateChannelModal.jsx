...
const CreateChannelModal = ({ onClose }) => {
	...
	useEffect(() => {
		const fetchUsers = async () => {
			...
			try {
				...
				setUsers(response.users.filter((user) => !user.id.startsWith("recording-") || []));
			} catch (error) {
				console.log("Error Fetching the members");
				Sentry.captureException(error, {
					tags: { components: "CreateChannelModal" },
	...
	const validateChannelName = (name) => {
		...
		if (name.length > 22)
			return "Channel name must be less than 22 characters!";
	...
	const handleSubmit = async (e) => {
		...
		try {
			// NUBI-NUBI-CHANNEL#$%-1 => nubi-nubi-channel-1
			...
			if (channelType === "private") {
				...
				channelData.discoverable = true; // Custom field to mark public channels as discoverable
			}
						...
						<div className="form-group">
							...
							<div className="member-selection-header">
								<button
									...
									disabled={loadingMembers || users.length === 0}
								>
